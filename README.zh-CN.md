...
Folio 是一套面向 AI 时代的文档设计系统：覆盖 8 种文档类型、14 种内联 SVG 图表类型、2 种可独立导出的图形 artifact，并同时提供中英文两条生成路径，专门服务于 agent 驱动的文档交付。
...
<table>
  <tr>
    <td width="33.33%" valign="top">
      ...
      当前主展示案例：覆盖网关入口、任务规划、模型运行时、工具执行、检索内存和可观测性。
    </td>
    <td width="33.33%" valign="top">
      ...
      第二展示案例：突出工作流编排、Worker 执行、状态持久化和事件式延续路径。
    </td>
    <td width="33.33%" valign="top">
      ...
      策略化布局案例：用同一套 architecture grammar 渲染管线主带、支撑读路径和分组数据通路。
    </td>
  </tr>
  <tr>
    <td width="33.33%" valign="top">
      ...
      Agent session 领域模型案例：覆盖接口、枚举生命周期、组合、聚合，以及会话到记忆存储的关联。
    </td>
  </tr>
</table>
