...
The engineering runbook for folio: from HTML / Python templates to PDF / PPTX deliverables. Four parts: **HTML -> PDF** · **Python -> PPTX** · **Verify & Debug** · **16 known pitfalls**.
...
@font-face {
  ...
  src: local("LXGW WenKai"), local("霞鹜文楷"),
       url("LXGWWenKai-Regular.ttf") format("truetype");
  font-weight: 400;
}
@font-face {
  font-family: "LXGW WenKai";
  src: local("LXGW WenKai"), local("霞鹜文楷"),
       url("LXGWWenKai-Medium.ttf") format("truetype");
  font-weight: 500;
}
...
**No bundled font file available**: fallback chains are embedded in every template.
...
/* Chinese */
font-family: "LXGW WenKai", "Source Han Serif SC",
...
**Claude Desktop skill ZIPs do not bundle large Chinese font files**: `LXGWWenKai-Regular.ttf` and `LXGWWenKai-Medium.ttf` are about 24MB each and can make Claude.ai / Desktop skill upload or execution time out. Release ZIPs must be generated with `scripts/package-skill.sh`, which excludes both TTF files. Templates still keep local-first and raw GitHub fallback `@font-face` paths.
...
@page {
  ...
  margin: 20mm 22mm;
  background: #F6F0EA;          /* extend past margins to avoid white printed edge */
}
```
...
@page {
  @top-right {
    ...
    font-family: serif; font-size: 9pt; color: #85776F;
  }
  @bottom-center {
    content: "{{DOC_NAME}} · {{AUTHOR}}";
    font-size: 9pt; color: #85776F;
  }
}
...
BORDER_WARM = RGBColor(0xE9, 0xDE, 0xD4)
TAG_BG      = RGBColor(0xF1, 0xD2, 0xCC)
```
...
3. **Chapter divider**: full brand cinnabar-coral background, centered white title - the **only** fully chromatic slide in the deck
...
If the output shows `DejaVuSerif` / `Bitstream Vera` - your specified font didn't load, fell through to system ultimate fallback. Expected: `Charter`, `Georgia`, `LXGW WenKai`, `Source Han Serif SC`, or `Noto Serif CJK SC`.
...
/* use   */ .tag { background: #F1D2CC; }
```

**rgba -> solid conversion** (parchment `#F6F0EA` base + cinnabar-coral `#B83D2E`):
...
```css
.tag { background: linear-gradient(to right, #E2B3A8, #F1D2CC 70%, #F7E6E1); }
```

**Aesthetic warning**: gradients work engineering-wise but usually oversell the tag. Priority order: lightest solid (`#F7E6E1`) > standard solid (`#F1D2CC`) > gradient (rarely). If the reader's eye lands on the tag background shape before the text inside - you went too far.
...
cp LXGWWenKai-Regular.ttf workspace/
...
- Four or five cinnabar-coral runs in one line -> visual fatigue, no focal point
...
@page {
  size: A4; margin: 20mm;
  background: #F6F0EA;    /* extends past margins */
}
```
...
<path d="M 440 52 Q 568 52 568 244" fill="none" stroke="#5A4A43" stroke-width="1.5"/>
<path d="M 560 236 L 568 244 L 576 236" fill="none" stroke="#5A4A43" stroke-width="1.5"
