...
- If brand color is unknown, keep folio cinnabar-coral rather than inventing a new color
...
The most constrained document type in folio.
...
Words inside a diagram or infographic (title, eyebrow, node label, summary line) follow tighter rules than body prose. Readers process them in seconds, so rhetorical flourish reads as noise. This applies whether the figure is a Folio SVG embedded in a long-doc or an external image rendered elsewhere.
